...
- For a Superpowers-authored multi-task plan, keep brainstorming, planning, and optional `superpowers:using-git-worktrees` on the architect side, but execute writing tasks through `/claude-architect:subagent-driven-delegation`, not generic `superpowers:subagent-driven-development`. One optional linked worktree is only the architect-owned branch home; every Producer attempt and No Mistakes validation uses its own managed worktree.
