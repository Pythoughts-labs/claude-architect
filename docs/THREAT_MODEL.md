...
| Acceptance/integration | `decideCandidate` requires positive MCP elicitation before `handleDecideCandidate` can record any decision; `handleIntegrateCandidate` requires an accepted record and exact manifest hash; `src/integrate/controlled-integrator.ts` rechecks base/ref/tree/status under locks | MCP elicitation is not cryptographic human identity; a compromised or modified trusted host/client could synthesize confirmation |
